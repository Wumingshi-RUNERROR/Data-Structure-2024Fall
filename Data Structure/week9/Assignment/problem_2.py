...
class Tree:
    ...
    def has_duplicates(self):
        ...
        def check(node,number):
            if node is None:
                return 0
            left=check(node._left,number)
            right=check(node._right,number)
            if node._element==number:
                return left+right+1
            else:
                return left+right
        ...
        def recur(node):
            if node is None:
                return False
            left=recur(node._left)
            right=recur(node._right)
            answer=check(self._root,node._element)>1
...
def main():
    ...
    print(t.has_duplicates())  # should print True
    ...
    print(t.has_duplicates())  # should print False
