...
class BinarySearchTree(Tree):
    ...
    def pairs(self, sum1):
        ...
        node1=self.first()
        node2=self.last()
        def recur(node1,node2,sum1):
            if node1._element>=node2._element:
                return None
            if node1._element+node2._element==sum1:
                return (node1._element,node2._element)
            elif node1._element+node2._element<sum1:
                return recur(self.after(node1),node2,sum1)
            else:
                return recur(node1,self.before(node2),sum1)
        return recur(node1,node2,sum1)
...
class Solution:
    def array_sum(self, arr, sum1):
        ...
        p2=len(arr)-1
        while p1<p2:
            if arr[p1]+arr[p2]==sum1:
                return (arr[p1],arr[p2])
            elif  arr[p1]+arr[p2]<sum1:
                p1+=1
            else:
                p2-=1
