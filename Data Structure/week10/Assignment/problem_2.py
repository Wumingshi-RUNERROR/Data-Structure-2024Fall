...
class Tree:
    ...
    def convert_to_bst(self):
        ...
        for node in self.preorder():
            array.append(node._element)
        array.sort()

        def recur(node):
            if node is None:
                return 0
            number1=recur(node._left)
            number2=recur(node._right)
            node._element=number1
            return number1+number2+1
        recur(self._root)

        def assign(node,low,high):
            if node is None:
                return 
            temp=node._element
            node._element=array[low+node._element]
            assign(node._left,low,low+temp-1)
            assign(node._right,low+temp+1,high)
        assign(self._root,0,len(array)-1)
...
class BinarySearchTree(Tree):
    ...
    def get_node(self, v):
        ...
        if self.is_empty():
            ...
            if v != node._element:
                raise Exception('Not found: ' + repr(v))
            return node
    ...
    def subtree_size(self):
        ...
        def recur(node):
            ...
            number1=recur(node._left)
            number2=recur(node._right)
            node._element[1]=number1+number2+1
            return node._element[1]
        recur(self._root)


def main():
    tree = BinarySearchTree()
    root = tree.add_root([8, 0])
    node = tree.add_right(root, [10, 0])
    node = tree.add_right(node, [14, 0])
    tree.add_left(node, [13, 0])

    node = tree.add_left(root, [3, 0])
    tree.add_left(node, [1, 0])
    node = tree.add_right(node, [6, 0])
    tree.add_right(node, [7, 0])
    tree.add_left(node, [4, 0])

    # Part A
    res = []
    for node in tree.preorder():
        res.append(node._element)
    print(res)  # Should print [[8, 0] [3, 0] [1, 0] [6, 0] [4, 0] [7, 0] [10, 0] [14, 0] [13, 0]]
    ...
    for node in tree.preorder():
        res.append(node._element)
    print(res)  # Should print [[8, 9], [3, 5], [1, 1], [6, 3], [4, 1], [7, 1], [10, 3], [14, 2], [13, 1]]


    tree = Tree()
    root = tree.add_root(8)
    node = tree.add_right(root, 3)
    node = tree.add_right(node, 1)
    tree.add_left(node, 2)

    node = tree.add_left(root, 12)
    tree.add_left(node, 18)
    node = tree.add_right(node, 5)
    tree.add_right(node, 7)
    tree.add_left(node, 4)

    # Part B
    res = []
    for node in tree.preorder():
        res.append(node._element)
    print(res)  # Should print [8, 12, 18, 5, 4, 7, 3, 1, 2]
    ...
    for node in tree.preorder():
        res.append(node._element)
    print(res)  # Should print [7, 2, 1, 4, 3, 5, 8, 18, 12]
