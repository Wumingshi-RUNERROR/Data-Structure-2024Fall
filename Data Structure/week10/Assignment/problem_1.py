...
class Tree:
    ...
    def is_bst(self):
        ...
        queue=LinkedQueue()
        queue.enqueue((self._root,-float('inf'),float('inf')))
        while not queue.is_empty():
            check=queue.dequeue()
            if check[0]._element>=check[1] and check[0]._element <=check[2]:
                if check[0]._left:
                    queue.enqueue((check[0]._left,check[1],check[0]._element))
                if check[0]._right:
                    queue.enqueue((check[0]._right,check[0]._element,check[2]))
            else:
                return False
        return True
...
def main():
    tree = Tree()
    tree.add_root(3)
    tree.add_left(tree.root(), 1)
    tree.add_right(tree.root(), 4)
    res = tree.is_bst()
    print(res)  # should print true

    tree = Tree()
    tree.add_root(10)
    tree.add_left(tree.root(), 15)
    tree.add_right(tree.root(), 8)
    res = tree.is_bst()
    print(res)  # should print false

    tree = BinarySearchTree()
    for node in [8, 3, 10, 1, 6, 4, 7, 14, 13]:
        tree.insert(node)
    res = tree.is_bst()
    print(res)  # should print true
